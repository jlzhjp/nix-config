...
- Enable XDG integration with `xdg.enable = true;` at the top level in `home.nix`.
...
- Neovim: Home Manager builds `config/nvim/init.fnl` into the installed `init.lua`; do not reintroduce runtime Fennel compilation.
...
- Fonts: keep standalone font packages in `modules/fonts.nix`, use `pkgs.nerd-fonts.fira-code` for Fira Code Nerd Font, and enable `fonts.fontconfig` when managing user fonts.
